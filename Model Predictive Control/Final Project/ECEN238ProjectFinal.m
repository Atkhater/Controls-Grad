...
for k = 1:(N - 1)
    ...
    Es(k) = Kp*D + Ki*intD;
    El = randn*sigma; % disturbance/noise
...
for k = 2:N
    ...
    E_opt2(k) = E_opt2(k-1) + Es_opt(k-1) + randn*sqrt(Wvar);

    % More Kalman filtering
    y(k) = E_opt2(k) + randn*sqrt(Vvar);
...
for k = 2:N
    Es_opt(k-1) = -Kinf*E_hat(k-1, 2) + Kcinf;
    E_opt3(k) = E_opt3(k-1) + Es_opt(k-1) + randn*sqrt(Wvar);
    y(k) = E_opt3(k) + randn*sqrt(Vvar);
